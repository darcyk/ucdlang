...
public final class EmailFileIngestModule implements FileIngestModule {
	...
	public final void startUp(IngestJobContext context) throws IngestModuleException {
		try {
			...
			// LOG.log(Level.INFO, "starting up email parder: " + settings.addToGraphDatabase() + "," +
			// settings.getNeo4jUriStr() + "," + settings.getNeo4jUsername() + "," + settings.getNeo4jPassword());
			if (settings.addToGraphDatabase()) {
				graphDb = new RestGraphDatabaseService(settings.getNeo4jUri().toString(), settings.getNeo4jUsername(),
						settings.getNeo4jPassword());
			}
		}
		catch (Throwable e) {
